...
public class Final extends Application{
	...
	public void start(Stage stage) {
		...
		for(int i = 0; i < 11; i++) {
			...
			for(int j = 0; j < 11; j++) {
				...
				if(i == 0 && j != 0) {
					...
					l.setFont(Font.font(20)); l.setTextFill(Color.BLACK);
					l.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
					l.setAlignment(Pos.CENTER); l.setBackground(bg2);
					gPane.add(l, i, j);
				}
				else if(j == 0 && i != 0) {
					...
					l.setFont(Font.font(17)); l.setTextFill(Color.BLACK);
					l.setMaxSize(Double.MAX_VALUE, Double.MAX_VALUE);
					l.setAlignment(Pos.CENTER); l.setBackground(bg4);
		...
		ColumnConstraints c = new ColumnConstraints(); c.setPercentWidth(10);
		RowConstraints r = new RowConstraints(); r.setPercentHeight(10);
		...
		tf1.setMaxWidth(100); tf1.setFocusTraversable(false);
		...
		tf2.setMaxWidth(40); tf2.setFocusTraversable(false);
		tf2.setAlignment(Pos.CENTER);
		tf2.setEditable(false); tf2.setCursor(Cursor.DEFAULT);
		
		Label lb3 = new Label("InstReg");
		lb3.setFont(Font.font(null, FontWeight.SEMI_BOLD, 17));
		lb3.setTextFill(Color.BLACK);
		tf3 = new TextField(String.format("%04d", 0));
		tf3.setFont(Font.font(17));
		tf3.setMaxWidth(80); tf3.setFocusTraversable(false);
		tf3.setAlignment(Pos.CENTER);
		tf3.setEditable(false); tf3.setCursor(Cursor.DEFAULT);
		
		Label lb4 = new Label("OpCode");
		lb4.setFont(Font.font(null, FontWeight.SEMI_BOLD, 17));
		lb4.setTextFill(Color.BLACK);
		tf4 = new TextField(String.format("%02d", 0));
		tf4.setFont(Font.font(17));
		tf4.setMaxWidth(40); tf4.setFocusTraversable(false);
		tf4.setAlignment(Pos.CENTER);
		tf4.setEditable(false); tf4.setCursor(Cursor.DEFAULT);
		
		Label lb5 = new Label("Operand");
		lb5.setFont(Font.font(null, FontWeight.SEMI_BOLD, 17));
		lb5.setTextFill(Color.BLACK);
		tf5 = new TextField(String.format("%02d", 0));
		tf5.setFont(Font.font(17));
		tf5.setMaxWidth(40); tf5.setFocusTraversable(false);
		tf5.setAlignment(Pos.CENTER);
		tf5.setEditable(false); tf5.setCursor(Cursor.DEFAULT);
		...
		b1.setOnAction(lmda -> {
			try{
				...
				if(!(file.getName().contains(".txt"))) {
					throw new FileNotFoundException("File selected is not a text file!");
				}
				if(Files.size(Paths.get(file.getPath())) == 0) {
			...
			catch (IOException | NullPointerException | InputMismatchException |
	...
	private void executeOne() {
        ...
        switch(opcode)
        {
            ...
            case READ: //Read a word into a specific location in memory
            	...
            	do {
            		try {
		            	dialog.setHeaderText("Enter a number");dialog.setTitle("Input Dialog");
	...
	private void reset() {
		...
		tf1.setText(String.format("%d", 0)); tf2.setText(String.format("%02d", 0));
		tf3.setText(String.format("%04d", 0)); tf4.setText(String.format("%02d", 0));
		tf5.setText(String.format("%02d", 0));
		accumulator = 0; instructionReg = 0; instCounter = 0; instSize = 0; opcode = 0; operand = 0;
		found = false;
		Arrays.fill(array, 0);
		list.clear();
	}
